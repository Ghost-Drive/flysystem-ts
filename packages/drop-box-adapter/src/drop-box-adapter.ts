...
export class DropboxAdapter implements IFlysystemAdapter {
    ...
    writeStream(pathOrId: PathOrId, resource: Readable, config?: VisibilityInterface | undefined): Promise<RequirePart<StorageItem, 'id' | 'isFolder' | 'name' | 'size'>> {
    ...
    setVisibility(pathOrId: PathOrId, visibility: VisibilityEnum): Promise<RequirePart<StorageItem, 'id' | 'isFolder' | 'name' | 'size'>> {
        throw new NotSupportedException('Adapter does not support visibility controls.');
    }
    ...
    async copy(source: PathOrId, destination: PathOrId, config?: Record<string, any> | undefined): Promise<RequirePart<StorageItem, 'id' | 'isFolder' | 'name'>> {
        ...
        try {
            ...
            return {
                ...
                id: (response.result.metadata as any).id,
                isFolder: false,
            };
        } catch (error) {
    ...
    async createDirectory(pathOrId: PathOrId, config?: VisibilityInterface | undefined) {
        ...
        const nativeRes = await this.dbx.filesCreateFolderV2({ path: location });

        return {
            id: nativeRes.result.metadata.id,
            name: nativeRes.result.metadata.name,
            isFolder: true,
        };
    }
    ...
    async delete(pathOrId: PathOrId) {
        ...
        try {
            ...
            const nativeRes = await this.dbx.filesDeleteV2({ path: location });

            return {
                id: (nativeRes.result.metadata as any).id,
                trashed: { permanently: true },
            };
        } catch (error) {
            return {};
        }
    ...
    deleteDirectory(pathOrId: PathOrId) {
    ...
    async move(source: PathOrId, destination: PathOrId, config?: Record<string, any> | undefined) {
        if (source.type !== 'id' || destination.type !== 'id') {
            throw new FlysystemException('ById API is not implemented yet');
        }
        ...
        try {
            const nativeRes = await this.dbx.filesMoveV2({
                from_path: fromPath,
                to_path: toPath,
            });
            ...
            return {
                id: (nativeRes.result.metadata as any).id,
                path: nativeRes.result.metadata.path_lower,
                isFolder: nativeRes.result.metadata['.tag'] === 'folder',
            };
        } catch (error) {
            throw new UnableToMoveFileException(`Unable to move <${fromPath}> to <${toPath}>`);
        }
    ...
    async write(pathOrId: PathOrId, contents: string | Buffer, config?: VisibilityInterface | undefined) {
        ...
        if (byteLength <= this.MAX_UPLOAD_PORTION) {
            const { result } = await this.dbx.filesUpload({ path: location, contents });
        ...
        const firstChunk = buff.slice(offset, end);
        ...
        while (true) {
            ...
            if (isFinish) {
                ...
                const { status, result } = await this.dbx.filesUploadSessionFinish({
